...
import { fillForm } from './enderecos-edit.js'
...
function goToEditPage(tr) {
    ...
    window.history.pushState(setSelectedEndereco(tr), '', '#edit/enderecos')
...
function apagar(tr) {
    ...
    updatableEmpresa.enderecos.splice(i, 1)
    deleteEndereco(updatableEmpresa)   
}

function deleteEndereco(updatableEmpresa) {
...
function addNew() {
    ...
    window.document.querySelector('div#edit').classList.add('active')
    window.document.getElementById('edit/enderecos').classList.add('active')
    window.history.pushState(empresa, '', '#edit/enderecos')
    fillForm()
}
