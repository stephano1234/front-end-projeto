...
import { fillForm } from './contas-edit.js'
...
function goToEditPage(tr) {
    ...
    window.history.pushState(setSelectedConta(tr), '', '#edit/contas')
...
function apagar(tr) {
    ...
    updatableEmpresa.contas.splice(i, 1)
    deleteConta(updatableEmpresa)   
}

function deleteConta(updatableEmpresa) {
...
function addNew() {
    ...
    window.document.querySelector('div#edit').classList.add('active')
    window.document.getElementById('edit/contas').classList.add('active')
    window.history.pushState(empresa, '', '#edit/contas')
    fillForm()
}
