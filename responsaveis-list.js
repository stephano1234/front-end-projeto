...
import { fillForm } from './responsaveis-edit.js'
...
function goToEditPage(tr) {
    ...
    window.history.pushState(setSelectedResponsavel(tr), '', '#edit/responsaveis')
...
function apagar(tr) {
    ...
    updatableEmpresa.responsaveis.splice(i, 1)
    deleteResponsavel(updatableEmpresa)   
}

function deleteResponsavel(updatableEmpresa) {
...
function addNew() {
    ...
    window.document.querySelector('div#edit').classList.add('active')
    window.document.getElementById('edit/responsaveis').classList.add('active')
    window.history.pushState(empresa, '', '#edit/responsaveis')
    fillForm()
}
