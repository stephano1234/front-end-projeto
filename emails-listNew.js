...
import { fillForm } from './emails-editNew.js'
...
function goToEditPage(tr) {
    ...
    window.history.pushState(setSelectedEmail(tr), '', '#editNew/emails')
...
function apagar(tr) {
    ...
    insertableEmpresa.emails.splice(i, 1)
    deleteEmail(insertableEmpresa)   
}

function deleteEmail(insertableEmpresa) {
...
function addNew() {
    ...
    window.document.querySelector('div#edit').classList.add('active')
    window.document.getElementById('edit/emails').classList.add('active')
    window.history.pushState(empresa, '', '#editNew/emails')
    fillForm()
}
