...
import { fillForm } from './telefonesFixo-edit.js'
...
function goToEditPage(tr) {
    ...
    window.history.pushState(setSelectedTelefoneFixo(tr), '', '#edit/telefonesFixo')
...
function apagar(tr) {
    ...
    updatableEmpresa.telefonesFixo.splice(i, 1)
    deleteTelefoneFixo(updatableEmpresa)   
}

function deleteTelefoneFixo(updatableEmpresa) {
    updateData(updatableEmpresa).then(
        () => {
            Object.assign(empresa, updatableEmpresa)
            fillTable(empresa)
            window.alert('Telefone Fixo apagado da lista com sucesso.')
        }
    )    
}
