...
import { fillForm } from './celulares-edit.js'
...
function goToEditPage(tr) {
    ...
    window.history.pushState(setSelectedCelular(tr), '', '#edit/celulares')
...
function apagar(tr) {
    ...
    updatableEmpresa.celulares.splice(i, 1)
    deleteCelular(updatableEmpresa)   
}

function deleteCelular(updatableEmpresa) {
...
function addNew() {
    ...
    window.document.querySelector('div#edit').classList.add('active')
    window.document.getElementById('edit/celulares').classList.add('active')
    window.history.pushState(empresa, '', '#edit/celulares')
    fillForm()
}
